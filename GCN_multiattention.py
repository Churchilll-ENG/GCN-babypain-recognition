...
class GraphConv_multi(nn.Module):
    def __init__(self, in_dim, out_dim, num_layers=2, hidden_dim=128):
        ...
        self.attention = CrossAttention(out_dim)
...
#        print(f"out:{out1.shape}")
        merged_out = self.attention(out1, out2, out3)
...
class CrossAttention(nn.Module):
    ...
    def attention(self, Q, K, V):
        d_k = Q.size(-1)
        scores = torch.matmul(Q, K.transpose(-2, -1)) / (d_k ** 0.5)  #matmul矩阵点积
    ...
    def forward(self, feat1, feat2, feat3):
        ...
        attn2 = self.attention(q1, k2, v2)  # 文本关注音频
        attn3 = self.attention(q1, k3, v3)  # 音频关注文本

        fusion = (attn2 + attn3) / 2
        fusion = self.out_proj(fusion)
        return fusion  # 让融合后的特征仍保留文本信息
