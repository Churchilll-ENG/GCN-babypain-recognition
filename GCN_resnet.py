...
class Graph_CNN_ortega(nn.Module):
    def __init__(self, num_layers, input_dim, hidden_dim, output_dim, final_dropout,
        ...
        ###List of resnet layers
        self.resnet = models.resnet50(weights=models.ResNet50_Weights.DEFAULT)
        self.resnet.conv1 = torch.nn.Conv2d(1, self.hidden_dim, kernel_size=7, stride=2, padding=3, bias=False)
        # 定义ResNet后面的线性层，使其输出与hidden_dim匹配
        self.resnet.fc = nn.Linear(self.resnet.fc.in_features, self.hidden_dim)  # 修改最后一个全连接层
        #self.resnet_linear = nn.Linear(self.resnet.fc.in_features, hidden_dim)
    ...
    def forward(self, batch_graph):
        ...
        for layer in self.GCNs:
            h = F.relu(layer(h, A, self.U))
        # 使用ResNet处理GCN的输出
        h=h.unsqueeze(1)
        resnet_output = self.resnet(h)  # 将GCN的输出作为ResNet的输入

        h=resnet_output
        # 应用线性层
#        h=self.resnet_linear(resnet_output.shape)
#        print(h.shape)
